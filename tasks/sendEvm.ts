...
const HOME_EID = EndpointId.OPTSEP_V2_TESTNET
const HUB_EID = EndpointId.ARBSEP_V2_TESTNET

// Stargate PoolNative addresses on chains we don't deploy our own contracts to
const STARGATE_NATIVE_POOLS: Record<number, string> = {
    [EndpointId.SEPOLIA_V2_TESTNET]: '0x9Cc7e185162Aa5D1425ee924D97a87A0a34A0706', // ETH Sepolia
}
...
/**
 * Get OApp contract address by EID from LayerZero config or external addresses
 */
async function getOAppAddressByEid(
    ...
    // Check external OFT addresses first (for chains we don't deploy to)
    if (STARGATE_NATIVE_POOLS[eid]) {
        logger.info(`Using Stargate PoolNative for EID ${eid}: ${STARGATE_NATIVE_POOLS[eid]}`)
        return STARGATE_NATIVE_POOLS[eid]
    }
...
export async function sendEvm(
    ...
    // Check if source is a Stargate PoolNative chain (e.g., ETH Sepolia)
    const isFromStargatePool = !!STARGATE_NATIVE_POOLS[srcEid]

    // Only allow sends FROM home/Stargate or TO home
    const isFromHome = srcEid === HOME_EID
    const isToHome = dstEid === HOME_EID
    if (!isFromHome && !isToHome && !isFromStargatePool) {
        throw new Error(
            `Sends must involve the home chain (${HOME_EID}) or originate from a Stargate pool. Got srcEid=${srcEid}, dstEid=${dstEid}`
        )
    }

    const getHreByEid = createGetHreByEid(hre)

    // Hub routing: only need hop if sending FROM home to a non-hub destination (spoke)
    // If sending FROM the hub, it can reach any destination directly
    const needsHop = srcEid !== HUB_EID && dstEid !== HUB_EID
    let actualDstEid = dstEid
    let actualTo = to
    let finalComposeMsg = composeMsg ?? '0x'

    if (needsHop) {
        logger.info(`Routing through hub (${HUB_EID}) to final destination (${dstEid})`)
        actualDstEid = HUB_EID

        // Get composer address from hub deployments
        const hubHre = await getHreByEid(HUB_EID)
        const composerDeployment = await hubHre.deployments.get('NativeStargateComposer')
        actualTo = composerDeployment.address

        // Stargate pools have enforced options, so only add lzReceive gas for non-Stargate destinations
        const isSecondHopToStargate = !!STARGATE_NATIVE_POOLS[dstEid]
        const secondHopOptions = isSecondHopToStargate
            ? '0x'
            : Options.newOptions().addExecutorLzReceiveOption(200000, 0).toHex()

        const secondHopSendParam = {
            dstEid: dstEid,
            to: addressToBytes32(to),
            amountLD: parseUnits(amount, 18), // Use actual amount for quoting
            minAmountLD: 0,
            extraOptions: secondHopOptions,
            composeMsg: '0x',
            oftCmd: '0x',
        }
        ...
        finalComposeMsg = hre.ethers.utils.defaultAbiCoder.encode(
            ...
            [
                [
                    [dstEid, addressToBytes32(to), 0, 0, secondHopOptions, '0x', '0x'],
                    [secondHopQuote.nativeFee, secondHopQuote.lzTokenFee],
                ],
            ]
        )
        logger.info(`Composer address: ${actualTo}`)
        logger.info(`Second hop to Stargate: ${isSecondHopToStargate}, extraOptions: ${secondHopOptions}`)
        logger.info(`Encoded HopParams for second hop to dstEid=${dstEid}`)
    }
    ...
    const dstEidHre = await getHreByEid(actualDstEid)
    const dstWrapperAddress = await getOAppAddressByEid(actualDstEid, oappConfig, dstEidHre, oftAddress)
    ...
    const toBytes = addressToBytes32(actualTo)
    ...
    if (isEmptyOptionsEvm(extraOptions)) {
        try {
            const enforcedOptions =
                finalComposeMsg !== '0x'
                    ? await oft.enforcedOptions(actualDstEid, MSG_TYPE.SEND_AND_CALL)
                    : await oft.enforcedOptions(actualDstEid, MSG_TYPE.SEND)
    ...
    const outboundNonce = (await _endpointContract.outboundNonce(wrapperAddress, actualDstEid, dstWrapperBytes32)).add(
        1
    )
